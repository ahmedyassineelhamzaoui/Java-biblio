		...
		registerButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(checkFeilds()) {
					...
					ResultSet rs;
					String Query ="Select * From users where cin=? or phone = ? or mail =?";
					try {
                        ...
                        if(rs.next()) {
                        	...
                        	PreparedStatement p;
        					ResultSet r;
        					String QueryInsert ="INSERT INTO users(name,cin,password,role,phone,mail,gender,image) VALUES(?,?,?,?,?,?,?,?)";
        					try {
        						p = ConnexionDB.getConnection().prepareStatement(QueryInsert);
            					p.setString(1, name);
            					p.setString(2, cin);
            					p.setString(3, password);
            					p.setString(4, "Emprunteur");
            					p.setString(5, tele);
            					p.setString(6, mail);
